...
def calculates_results_stats(results_dic):
    ...
    results_stats_dic = {}
        ...
        # Calculating statistics related to images and classifications
    n_images = len(results_dic)
    n_dogs_img = sum([results_dic[key][3] for key in results_dic])
    n_match = sum([results_dic[key][2] for key in results_dic])

    n_correct_dogs = sum([1 for key in results_dic if results_dic[key][3] and results_dic[key][4]])
    n_correct_breed = sum([1 for key in results_dic if results_dic[key][2] and results_dic[key][3]])
    n_correct_notdogs = sum([1 for key in results_dic if not results_dic[key][3] and not results_dic[key][4]])

        # Calculate the number of NON-dog images
    n_notdogs_img = n_images - n_dogs_img

        # Populate results_stats_dic with counts
    ...
    results_stats_dic['n_correct_breed'] = n_correct_breed
        ...
        # Calculate percentages and store them in results_stats_dic
